...
const issues: string[] = [];
const siteUrl = "https://marevo-zadar.vercel.app";

for (const path of ["/", "/search", ...experiences.map((experience) => `/experiences/${experience.slug}`)]) {
...
if (!robots.includes(`Sitemap: ${siteUrl}/sitemap.xml`)) issues.push("robots.txt sitemap URL is missing");
if (manifest.name !== "MAREVO — Zadar from the sea") issues.push("Manifest name is incorrect");
