        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((c, i) => (
            <Link
              ...
              style={{ borderBottomRightRadius: i % 2 === 0 ? "4rem" : undefined, borderTopLeftRadius: i % 2 === 1 ? "4rem" : undefined }}
            >
              ...
              <div
                ...
                style={{ background: "linear-gradient(180deg, transparent 35%, oklch(0.267 0.043 223.7 / 0.78) 100%)" }}
              />
...
export function Destinations() {
  return (
    <section id="destinations" className="mx-auto max-w-[1240px] scroll-mt-24 px-5 py-20 sm:px-8 lg:py-28">
      <div className="max-w-xl">
        ...
        <p className="mt-4 leading-relaxed text-muted-foreground">
          Zadar sits in front of a wall of islands. Everything below is a day trip, and most of it is unreachable
          without a boat.
        </p>
      </div>
          ...
          >
            ...
            <div className="flex flex-col justify-center p-5">
              ...
              <span className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-ink">
                See trips <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" aria-hidden="true" />
              </span>
            </div>
          </Link>
        ))}
      ...
      <div className="mx-auto grid max-w-[1240px] items-center gap-12 px-5 sm:px-8 lg:grid-cols-2">
        <div>
          ...
          <p className="mt-4 max-w-md leading-relaxed text-ink/75">
            Describe the day you have in mind and we match it against the boats we actually know — capacity, shade,
            departure point, how far the skipper is willing to go and what the wind is doing that week. A person from
            the Zadar team checks every match before it reaches you.
          </p>
        ...
        <figure className="relative rounded-xl border border-ink/10 bg-background p-6 shadow-soft sm:p-8">
          <Quote className="absolute -top-3 -left-3 h-8 w-8 rounded-full bg-sun p-1.5 text-ink" aria-hidden="true" />
          ...
          <blockquote className="mt-3 font-display text-xl leading-relaxed text-balance sm:text-2xl">
            “We're 6 people in Zadar tomorrow. Private boat, lots of swimming, somewhere quiet, max €700.”
          </blockquote>
          <div className="mt-6 flex flex-wrap gap-2 border-t border-border pt-5 text-xs">
            {["6 guests", "Private", "Quiet coves", "Under €700", "Tomorrow"].map((t) => (
              <span key={t} className="rounded-full border border-border bg-secondary px-3 py-1 text-ink/80">
                {t}
              </span>
            ))}
          </div>
...
const steps = [
  { icon: CalendarCheck, title: "Choose your day", text: "Pick a trip, a date and your group size, then send a request with anything the skipper should know." },
  { icon: CheckCircle2, title: "Operator confirms", text: "The owner checks the boat, the crew and the forecast, then replies — usually within 30 minutes." },
  { icon: CreditCard, title: "Pay securely", text: "Only once the boat is confirmed do you pay. Free cancellation applies on selected trips." },
];
        ...
        {steps.map((s, i) => (
          <li key={s.title} className="relative">
            <div className="flex items-center gap-3">
              <span className="grid h-10 w-10 place-items-center rounded-full bg-ink text-sm text-background">{i + 1}</span>
      ...
      <div className="mx-auto grid max-w-[1240px] items-center gap-14 px-5 sm:px-8 lg:grid-cols-2">
        <div className="cove border border-background/10">
          <img
            ...
            alt="A local skipper preparing his boat in the harbour at first light"
            loading="lazy"
        ...
        <div>
          ...
          <p className="mt-5 leading-relaxed text-background/75">
            MAREVO started because good local skippers were losing their season to platforms that treat a boat like a
            hotel room. We work with a small number of families who own their boats, know the channels and answer their
            own phones. If a bay is crowded, they will take you somewhere else. If the bura comes, they will tell you
            honestly.
          </p>
          <p className="mt-4 leading-relaxed text-background/75">
            No fleet resellers, no last-minute substitutions, no boat you have never seen a photograph of.
          </p>
        ...
        <Accordion type="single" collapsible className="w-full">
          {faqs.map((f, i) => (
            <AccordionItem key={f.q} value={`item-${i}`} className="border-ink/10">
              ...
              <AccordionContent className="pb-5 text-[0.95rem] leading-relaxed text-ink/70">{f.a}</AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
      </div>
  ...
  return (
    <section className="mx-auto max-w-[1240px] px-5 py-20 sm:px-8">
      <div className="grid items-center gap-8 rounded-xl border border-border bg-card p-8 sm:p-12 lg:grid-cols-[1fr_auto]">
        <div>
          <h2 className="font-display text-2xl leading-tight font-medium sm:text-3xl">Not ready to choose?</h2>
          <p className="mt-2 max-w-md leading-relaxed text-muted-foreground">
            One short email a month: new boats, quieter weeks and the best days to be on the water around Zadar.
          </p>
        </div>
        {done ? (
          ...
          >
            <div className="sm:w-64">
              ...
              <Input id="newsletter" type="email" required placeholder="you@example.com" className="h-11" />
            </div>
